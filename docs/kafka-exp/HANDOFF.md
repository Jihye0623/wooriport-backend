...
|------|-------------|---------------|----------|-------|
| consumer 처리량(정상상태, 클린 시드) | ~301 msg/s | ~248 msg/s | **~1,305 msg/s** | **+426% (5.3×)** |
| tx.persist p50 (메시지당) | 2.49 ms | 3.15 ms | **0.121 ms** | **−96%** |
...
> **Phase 2 는 `@KafkaListener(batch=true)` + IN dedup 1쿼리 + saveAll 배치 INSERT 로 −18% 회복 + v0 대비 ~4.3× 초과.**
> 정합성(멱등·재처리·재계산)은 트랙 B 그대로 통과. 측정일 2026-06-08, 상세 phase-2.md.
> **병목 이동**: persist 는 배치로 제거됨(100k 전체 12.08s) → 이제 메시지당 JSON 파싱 + 컨슈머 poll 루프(단일 스레드)가 한계 → **Phase 3 = concurrency↑**.
> ⚠️ **측정 전 Redis FLUSHALL + `demo_chaos_seed` 제거 필수**(활성 챌린지가 남으면 처리량 ~1,305→~1,118 로 떨어짐 — phase-2.md 민감도 표 참고).
> ⚠️ **절대값(301/248/1,305)은 이 머신 전용**. 하드웨어 바뀌면 §4 참고.
