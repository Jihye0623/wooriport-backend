            ...
            steps {
                sh '''
                    COMMAND_ID=$(aws ssm send-command \
                        ...
                        --parameters '{"commands":["export HOME=/root && git config --global --add safe.directory /opt/backend && cd /opt/backend && git pull origin service && docker compose up --build -d"]}' \
